...
One agent action, traced through the whole control plane — a human signs, the agent is bound to a non-spoofable identity, the Token Exchange fuses them, every call passes a PEP, and every governed event lands in the tamper-evident audit chain. Press play to watch the packet walk the path.
